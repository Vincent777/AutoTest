...
if [ -n "$PD_TOPOLOGY" ]; then
    ...
    if [ "$PD_SGLANG_TRANSFER_BACKEND" = "ascend" ] && [ "$PD_ROLE" != "proxy" ]; then
        ...
        # 补丁由 ascend_resource_monitor scp 到各 NPU 的 /home/s_limingge/（推理容器只挂该目录，看不到 /CI_Workspace）
        PD_PIP_BOOTSTRAP="${PD_PIP_BOOTSTRAP} python3 /home/s_limingge/pd_ascend_send_kvcache_compat.py || exit 1; "
    fi
...
ret=`docker ps -a | grep sglang_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX}`
if [ $? -eq 0 ]; then
    docker stop sglang_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX}
    docker rm sglang_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX}
fi
...
EXEC_COMMAND="docker run --name=sglang_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX} \
  --network host \
  --ipc=host \
  --privileged \
...
if [ $MODEL == "DeepSeek-R1-Distill-Qwen-32B" ]; then
    echo "python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Qwen-32B --served-model-name DeepSeek-R1-Distill-Qwen-32B --port $PORT --tp-size 2 --host 0.0.0.0 --context-length 32768 --disable-radix-cache --enable-metrics --enable-mfu-metrics $PD_EXTRA_ARGS"
    EXEC_COMMAND+=" $PD_DOCKER_CMD_PREFIX python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Qwen-32B --served-model-name DeepSeek-R1-Distill-Qwen-32B --port $PORT --tp-size 2 --host 0.0.0.0 --context-length 32768 --disable-radix-cache --enable-metrics --enable-mfu-metrics $SGLANG_PREFIX_CACHE $PD_EXTRA_ARGS $PD_DOCKER_CMD_SUFFIX > $LOG_NAME 2>&1 &"
elif [ $MODEL == "DeepSeek-R1-Distill-Llama-8B" ]; then
    echo "python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Llama-8B --served-model-name DeepSeek-R1-Distill-Llama-8B --port $PORT --tp-size 1 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $PD_EXTRA_ARGS"
    EXEC_COMMAND+=" $PD_DOCKER_CMD_PREFIX python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Llama-8B --served-model-name DeepSeek-R1-Distill-Llama-8B --port $PORT --tp-size 1 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $SGLANG_PREFIX_CACHE $PD_EXTRA_ARGS $PD_DOCKER_CMD_SUFFIX > $LOG_NAME 2>&1 &"
elif [ $MODEL == "Qwen3-32B-FP8" ]; then
    echo "python3 -m sglang.launch_server --model-path /home/weight/Qwen3/Qwen3-32B-FP8 --served-model-name Qwen3-32B-FP8 --port $PORT --tp-size 2 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $PD_EXTRA_ARGS"
    EXEC_COMMAND+=" $PD_DOCKER_CMD_PREFIX python3 -m sglang.launch_server --model-path /home/weight/Qwen3/Qwen3-32B-FP8 --served-model-name Qwen3-32B-FP8 --port $PORT --tp-size 2 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $SGLANG_PREFIX_CACHE $PD_EXTRA_ARGS $PD_DOCKER_CMD_SUFFIX > $LOG_NAME 2>&1 &"
elif [ $MODEL == "DeepSeek-R1-Distill-Llama-70B" ]; then
    echo "python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Llama-70B --served-model-name DeepSeek-R1-Distill-Llama-70B --port $PORT --tp-size 4 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $PD_EXTRA_ARGS"
    EXEC_COMMAND+=" $PD_DOCKER_CMD_PREFIX python3 -m sglang.launch_server --model-path /home/weight/DeepSeek-R1-Distill-Llama-70B --served-model-name DeepSeek-R1-Distill-Llama-70B --port $PORT --tp-size 4 --host 0.0.0.0 --disable-radix-cache --enable-metrics --enable-mfu-metrics $SGLANG_PREFIX_CACHE $PD_EXTRA_ARGS $PD_DOCKER_CMD_SUFFIX > $LOG_NAME 2>&1 &"
    ...
    echo "python3 -m sglang.launch_server --model-path /home/weight/MiniMax-M2.5-w8a8-QuaRot --served-model-name MiniMax-M2.5-w8a8-QuaRot --port $PORT --tp-size 8 --host 0.0.0.0 --attention-backend ascend --device npu --trust-remote-code --mem-fraction-static 0.85 --tool-call-parser minimax-m2 --reasoning-parser minimax-append-think --quantization modelslim --disable-radix-cache $PD_EXTRA_ARGS"
    EXEC_COMMAND+=" $PD_DOCKER_CMD_PREFIX python3 -m sglang.launch_server --model-path /home/weight/MiniMax-M2.5-w8a8-QuaRot --served-model-name MiniMax-M2.5-w8a8-QuaRot --port $PORT --tp-size 8 --host 0.0.0.0 --attention-backend ascend --device npu --trust-remote-code --mem-fraction-static 0.85 --tool-call-parser minimax-m2 --reasoning-parser minimax-append-think --quantization modelslim --disable-radix-cache $SGLANG_PREFIX_CACHE $PD_EXTRA_ARGS $PD_DOCKER_CMD_SUFFIX > $LOG_NAME 2>&1 &"
fi
