...
if [ -z $ENGINE_TYPE ]; then
    ...
    exit 1
elif [ $ENGINE_TYPE != "SigInfer" ]; then
    echo "Inference Engine Type is wrong! Only SigInfer is supported on Iluvatar."
    exit 1
fi
...
echo "#################################### Iluvatar #####################################"
echo "$TEST_TYPE $ENGINE_TYPE $MODEL_LIST $SESSION_ID $TEST_PARAM $version"
echo "#################################################################################"

# PD 分离（可选）：PD_TOPOLOGY=2P2D 时按「同角色可共机、P/D 不可混部」选机
# 默认允许同角色堆叠（跨 CI job 通过 host_role_lease 共享主机角色）
PD_TOPOLOGY="${PD_TOPOLOGY:-}"
# 天数智芯环境默认不启用 PD（缺少 pd_place_servers / host_role_lease 时强制关闭）
if [ -n "$PD_TOPOLOGY" ] && { [ ! -f "$curr_dir/pd_place_servers.py" ] || [ ! -f "$curr_dir/host_role_lease.py" ]; }; then
    echo "WARN: PD helpers missing; ignore PD_TOPOLOGY=$PD_TOPOLOGY"
    PD_TOPOLOGY=""
fi
...
full_model_list_for_smoke=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:8 Qwen3-30B-A3B:2)
full_model_list_for_performance=(DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-0528:16 Qwen3-235B-A22B:8 Qwen3-32B:8 Qwen2.5-72B-Instruct:4 Meta-Llama-3.1-70B-Instruct:4)
# full_model_list_for_performance=(DeepSeek-R1-0528:16 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Qwen3-32B:8 Qwen3-235B-A22B:8 DeepSeek-R1-W8A8:16)
full_model_list_for_accuracy=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:8 Qwen3-30B-A3B:2)
full_model_list_for_stability=(DeepSeek-R1-0528:16 Qwen3-235B-A22B:8 Meta-Llama-3.1-70B-Instruct:4 Qwen3-32B:8)
...
search_servers() {
    ...
    for key in "${!npu_server_list[@]}"; do
        ...
        ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  root@${npu_server_list[$key]} "# 目标空闲 GPU 数量
            if [ -f /root/npu_lock_manager_for_ci.sh ]; then
                source /root/npu_lock_manager_for_ci.sh
            else
                source /root/npu_lock_manager_for_ci.sh
            fi
            ...
            # CoreX ixsmi 需要 libixml.so
            export LD_LIBRARY_PATH=/usr/local/corex/lib64:/usr/local/corex/lib:\${LD_LIBRARY_PATH:-}
            IXSMI_BIN=/usr/local/corex/bin/ixsmi
            if [ ! -x \"\$IXSMI_BIN\" ]; then
                IXSMI_BIN=\$(command -v ixsmi 2>/dev/null || true)
            fi
            if [ -z \"\$IXSMI_BIN\" ] || ! \"\$IXSMI_BIN\" -L >/dev/null 2>&1; then
                echo \"错误: ixsmi 无法运行（缺少 libixml.so 或未安装 CoreX） LD_LIBRARY_PATH=\$LD_LIBRARY_PATH\"
                \"\$IXSMI_BIN\" -L 2>&1 | head -n 3 || true
                exit 1
            fi
            BUSY_GPUS=(\$(\"\$IXSMI_BIN\" | awk '/Processes:/,/\\+/{ if (\$1 ~ /^[|]/ && \$2 ~ /^[0-9]+\$/) print \$2 }'))
...
search_pd_servers() {
    ...
    for key in "${!npu_server_list[@]}"; do
        ...
        local probe_out
        probe_out=$(ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3  root@${host_ip} "
            if [ -f /root/npu_lock_manager_for_ci.sh ]; then
                source /root/npu_lock_manager_for_ci.sh
            else
                source /root/npu_lock_manager_for_ci.sh
...
for name in "${!npu_server_list[@]}"; do
    ...
    scp  "${curr_dir}/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh" root@${npu_server_list[$name]}:/root/
    scp  "${curr_dir}/npu_lock_manager_for_ci.sh" root@${npu_server_list[$name]}:/root/
    if [ -f "${curr_dir}/pd_ascend_send_kvcache_compat.py" ]; then
        scp  "${curr_dir}/pd_ascend_send_kvcache_compat.py" root@${npu_server_list[$name]}:/root/
    fi
done
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ] && [ ${#servers[@]} -gt 0 ]; then
            ...
            if [ $TEST_TYPE == "Stability" ]; then
                ...
                $curr_dir/siginfer_iluvatar_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/stability/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_iluvatar_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${TEST_PARAM} ${version} > $curr_dir/logs/performance/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_iluvatar_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/smoke/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_iluvatar_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/accuracy/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
