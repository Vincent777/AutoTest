...
ret=`docker ps -a | grep siginfer_moore_PerformanceTest_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_moore_PerformanceTest_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_moore_PerformanceTest_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_moore_PerformanceTest_${SESSION_ID}_${JOB_COUNT} \
     -u root  \
     --privileged \
     --ipc=host \
     --network host \
...
if [ $MODEL == "DeepSeek-R1-AWQ" ]; then
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model Qwen3.5-27B --tokenizer /home/weight/Qwen3.5-27B -tp 2 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 2048 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --platform-type moore $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT"
    EXEC_COMMAND+=" --model Qwen3.5-27B --tokenizer /home/weight/Qwen3.5-27B -tp 2 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 2048 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --platform-type moore $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT > $LOG_NAME 2>&1 &"
elif [ $MODEL == "Qwen3.5-35B-A3B" ]; then
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model Qwen3.5-35B-A3B --tokenizer /home/weight/Qwen3.5-35B-A3B -tp 1 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 2048 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --platform-type moore $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT"
    EXEC_COMMAND+=" --model Qwen3.5-35B-A3B --tokenizer /home/weight/Qwen3.5-35B-A3B -tp 1 --block-size 128 --weight-dtype=FP16 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 2048 --port $PORT --master-addr $MASTER_IP --master-port $MASTER_PORT --nnodes 1 --node-rank $NODE_RANK --platform-type moore $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT > $LOG_NAME 2>&1 &"
fi
