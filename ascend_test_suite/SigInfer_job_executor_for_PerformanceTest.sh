...
ret=`docker ps -a | grep siginfer_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_ascend_PerformanceTest_${SESSION_ID}_${JOB_COUNT} \
     -u root  \
