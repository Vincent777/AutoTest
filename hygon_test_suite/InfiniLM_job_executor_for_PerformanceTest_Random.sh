...
# 生成唯一的任务ID
TASK_ID="PerformanceTest_${MODEL}_${OPTIONS}_${JOB_COUNT}"
JOB_ID="PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}"
...
cleanup_locks() {
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        if [[ "PerformanceTest" =~ ^(ServiceTest|PerformanceTest|SmokeTest|AccuracyTest|StabilityTest)$ ]]; then
...
ret=`docker ps -a | grep infiniLM_hygon_PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniLM_hygon_PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniLM_hygon_PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
fi
...
LOG_PATH="/data-aisoft/artifacts"
LOG_NAME="server_log_PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}_$(date +'%Y%m%d_%H%M%S').log"
...
EXEC_COMMAND="docker run --name=infiniLM_hygon_PerformanceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT} "
EXEC_COMMAND+="-e HIP_VISIBLE_DEVICES=$HIP_VISIBLE_DEVICES "
EXEC_COMMAND+=$(cat <<EOF
    --rm --network=host --ipc=host --privileged -v /data-aisoft/mechdancer/models:/data-aisoft/mechdancer/models -v /data-aisoft/artifacts:/artifacts $DOCKER_IMAGE_URL python InfiniLM/python/infinilm/server/inference_server.py
EOF
)

if [ "${MODEL}_${GPU_MODEL}" == "9g_8b_thinking_Z100L" ]; then
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --device hygon --model=/data-aisoft/mechdancer/models/9g_8b_thinking/ --max-new-tokens=100 --max-batch-size=32 --tp=1 --temperature=1.0 --top-p=0.8 --top-k=1 --port $PORT"
    EXEC_COMMAND+=" ./start.sh --device hygon --model=/data-aisoft/mechdancer/models/9g_8b_thinking/ --max-new-tokens=100 --max-batch-size=32 --tp=1 --temperature=1.0 --top-p=0.8 --top-k=1 --port $PORT > $LOG_PATH/$LOG_NAME 2>&1 &"
fi
