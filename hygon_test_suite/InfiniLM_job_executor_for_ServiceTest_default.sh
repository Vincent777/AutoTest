...
# 生成唯一的任务ID
TASK_ID="ServiceTest_${MODEL}_${OPTIONS}_${JOB_COUNT}"
JOB_ID="ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}"
...
cleanup_locks() {
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        if [[ "ServiceTest" =~ ^(ServiceTest|PerformanceTest|SmokeTest|AccuracyTest|StabilityTest)$ ]]; then
...
ret=`docker ps -a | grep infiniLM_hygon_ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop infiniLM_hygon_ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
  docker rm infiniLM_hygon_ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}
fi
...
LOG_NAME="server_log_ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT}_$(date +'%Y%m%d_%H%M%S').log"
...
EXEC_COMMAND="docker run --name=infiniLM_hygon_ServiceTest_${MODEL}_${OPTIONS}_${SESSION_ID}_${JOB_COUNT} "
