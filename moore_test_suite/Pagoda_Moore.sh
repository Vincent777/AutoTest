#!/usr/bin/env bash
# Pagoda_Moore.sh — 摩尔线程 SigInfer 一键入口（对齐 ascend Pagoda_910B*.py / hygon Pagoda_HYGON.sh）
#
# 用法:
#   ./Pagoda_Moore.sh --test_type Service
#   ./Pagoda_Moore.sh --test_type Performance
#
# 可选参数:
...
usage() {
    cat <<'EOF'
Pagoda_Moore.sh — 摩尔线程 SigInfer 一键入口（对齐 ascend Pagoda_910B*.py）
...
MONITOR="$SCRIPT_DIR/moore_resource_monitor.sh"
chmod +x "$MONITOR" "$SCRIPT_DIR/siginfer_moore_test.sh" 2>/dev/null || true

echo "========================================"
echo " Pagoda Moore / SigInfer"
...
cleanup() {
    echo "Shutting down Pagoda_Moore..."
...
set +e
# 对齐 ascend: bash moore_resource_monitor.sh <Stability|Performance> SigInfer <model> <session> [Random] [version]
