...
if [[ "$TEST_TYPE" =~ ^(Service|Performance|Smoke|Accuracy|Stability)$ ]]; then
    ...
    mkdir -p "${curr_dir}/logs/${test_type}/$session_id"
    ...
    for item in "${model_list[@]}"; do
        ...
        for ip in ${server_list[@]}; do
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip chmod a+x /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh $model $gpu_quantity "${OPTIONS}" $server_list_str $seq_num $job_count $gpu_model $session_id $version > "$curr_dir/logs/smoke/$session_id/${filename}_${seq_num}" &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            else
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip chmod a+x /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh $model $gpu_quantity "${OPTIONS}" $server_list_str $seq_num $job_count $gpu_model $session_id $version &
                ssh_pid=$!
                pid_map[$ssh_pid]=$ip
                SSH_PID_MAP[$ssh_pid]=$ip
            fi
                ...
                if [ $err -ne 0 ]; then
                    ...
                    for ip in ${server_list[@]}; do
                        if [ $ENGINE_TYPE == "InfiniLM" ]; then
                            ssh -q -o ConnectionAttempts=3  root@$ip docker stop infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 root@$ip docker rm infiniLM_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        elif [ $ENGINE_TYPE == "vLLM" ]; then
                            ssh -q -o ConnectionAttempts=3 root@$ip docker stop vllm_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                            ssh -q -o ConnectionAttempts=3 root@$ip docker rm vllm_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}
                        fi
                    done
        ...
        if [ $TEST_TYPE == "Service" ]; then
            ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$local_master_ip "
            ...
            bench_container="${ENGINE_TYPE,}_hygon_${TEST_TYPE}Test_${model}_${OPTIONS}_${session_id}_${job_count}"
            mkdir -p "$curr_dir/logs/performance/$session_id"
            # 对齐 ascend Performance：Random / SharedGPT 矩阵，输出格式兼容 WriteReportToExcel
                    ...
                    docker exec ${bench_container} /bin/bash -c \"
                        ...
                        for pair in ${length_pairs[@]}; do
                            ...
                            for concurrency in ${concurrency_list[@]}; do
                                ...
                                echo \\\"python InfiniLM/scripts/test_perf.py --api-url 127.0.0.1:${server_port} --input-len \\\$input_len --output-len \\\$output_len --num-prompts \\\$prompts --max-concurrency \\\$concurrency --verbose\\\"
                                if python InfiniLM/scripts/test_perf.py --help 2>&1 | grep -q -- '--input-len'; then
                                    python InfiniLM/scripts/test_perf.py \\
                                        --api-url 127.0.0.1:${server_port} \\
                                        --input-len \\\$input_len \\
                                    ...
                                    # 旧版 test_perf 无矩阵参数时，至少跑一轮并打印兼容表头
                                    python InfiniLM/scripts/test_perf.py --api-url 127.0.0.1:${server_port} --verbose
                                fi
                            done
                        done
                    \"
                " > "$curr_dir/logs/performance/$session_id/$filename" 2>&1
            else
                ...
                ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@${local_master_ip} "
                    docker exec ${bench_container} /bin/bash -c \"
                        ...
                        for concurrency in ${concurrency_list[@]}; do
                            ...
                            echo \\\"Testing concurrency=\\\$concurrency, prompts=\\\$prompts\\\"
                            echo \\\"python InfiniLM/scripts/test_perf.py --api-url 127.0.0.1:${server_port} --dataset sharegpt --num-prompts \\\$prompts --max-concurrency \\\$concurrency --verbose\\\"
                            if python InfiniLM/scripts/test_perf.py --help 2>&1 | grep -q -- '--dataset'; then
                                python InfiniLM/scripts/test_perf.py \\
                                    --api-url 127.0.0.1:${server_port} \\
                                    --dataset sharegpt \\
                                    --num-prompts \\\$prompts \\
            ...
            tail -n 100 "$curr_dir/logs/performance/$session_id/$filename" || true
            ...
            # 对齐 ascend Pagoda Service(Stability)：服务保持运行直到手动中断
            echo "Stability mode: service keeps running. Press Ctrl+C to stop......"
            sleep infinity
        fi
        ...
        if [ $send_report -eq 1 ]; then
            ...
            if [ $TEST_TYPE == "Smoke" ]; then
                ...
                echo "$latest_tag" > "$curr_dir/report_${log_name_suffix}/$session_id/version.txt"
                # 获取模型启动命令，并做为参数传入
                exec_cmd=`cat "$curr_dir/logs/performance/$session_id/cron_job_${OPTIONS}_${log_name_suffix}_${job_count}.log" 2>/dev/null | grep "docker run" | head -n 1`
                # 获取测试命令，并做为参数传入
                if [ $ENGINE_TYPE == "InfiniLM" ]; then
                    test_cmd=`cat "$curr_dir/logs/performance/$session_id/$filename" | grep -E "test_perf.py|benchmark_serving.py" | head -n 1 | sed -E 's/--(random-input-len|random-output-len|input-len|output-len|num-prompts|max-concurrency)\s+[0-9]+/--\1 xxx/g'`
                elif [ $ENGINE_TYPE == "vLLM" ]; then
                ...
                # 生成本次测试的Excel报告
                if [ -f "$curr_dir/logs/performance/$session_id/$filename" ]; then
                    python3 $curr_dir/WriteReportToExcel.py "$TEST_PARAM" "${model}" "$session_id" "$exec_cmd" "$test_cmd" "$curr_dir/logs/performance/$session_id/$filename" || true
                    CI_report_folder="/artifacts/CI_hygon_test/${session_id}_hygon_npu_performancetest"
                    if [ -d "/artifacts" ]; then
                        mkdir -p "$CI_report_folder"
                        if [ -f "$curr_dir/report_${log_name_suffix}/$session_id/${model}.xlsx" ]; then
    ...
    ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip chmod a+x /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh
    ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip /root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test_${OPTIONS}.sh $model $gpu_quantity "${OPTIONS}" $server_list_str 0 0 $gpu_model $session_id $version > "$curr_dir/logs/${test_type}/$session_id/${filename}" &
    ssh_pid=$!
    pid_map[$ssh_pid]=$ip
    SSH_PID_MAP[$ssh_pid]=$ip
