...
ret=`docker ps -a | grep siginfer_hygon_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_hygon_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_hygon_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_hygon_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT} \
     -u root  \
     --privileged \
     --ipc=host \
     --network host \
     ...
     --volume /workspace/siginfer-install:/workspace/siginfer-install \
     --device=/dev/kfd \
     --device=/dev/dri \
     --cap-add=SYS_PTRACE \
     ...
     -e LD_LIBRARY_PATH=/workspace/siginfer-install/lib:$LD_LIBRARY_PATH \
