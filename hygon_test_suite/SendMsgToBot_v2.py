...
def read_and_format_summary_v2(file_path, version_text, compare_result):
    """
    读取summary文件并格式化输出
    
    Args:
        file_path: summary文件路径
        version_text: 上一个版本信息
        
    Returns:
        格式化后的字符串，第一行显示版本信息，第二行显示列标题，
        第一列模型名称按最长名称宽度对齐，冒号后面是5列：passed, failed, warning, skipped, comparison
    """
    def get_diff_passed(model_name):
        if model_name in compare_result:
            return compare_result[model_name]['diff_passed']
        else:
            return None
        ...
        if ':' in line:
            ...
            if len(numbers) >= 4:
                parsed_data.append({
                    ...
                    'warning': numbers[2],
                    'skipped': numbers[3],
                    'comparison': get_diff_passed(model_name)
                })
                # 更新最长模型名称长度
                max_model_name_length = max(max_model_name_length, len(model_name))
    
    dict1 = {item['model_name']: item for item in parsed_data}
    for model_name in compare_result:
        if model_name not in dict1:
            parsed_data.append({
                'model_name': model_name,
                'passed': 'N/A',
                'failed': 'N/A',
    ...
    # 列标题（等宽对齐）
    header_line = f"{'Model Name':<{max_model_name_length}} :{'Passed':>8} {'Failed':>8} {'Warning':>8} {'Skipped':>8} {f'Comparison({version_text})':>8}"
    formatted_lines.append(header_line)
    
    # 数据行
    for data in parsed_data:
        # 模型名称列按最长名称宽度对齐，后面是4列数据
        formatted_line = f"{data['model_name']:<{max_model_name_length}} :{data['passed']:>8} {data['failed']:>8} {data['warning']:>8} {data['skipped']:>8} {data['comparison']:>8}"
...
def compare_summary_files_v2(file_path1, file_path2):
    """
    ...
    for model_name in all_model_names:
        if model_name in dict1 and model_name in dict2:
            ...
            result[model_name] = {
                            'diff_passed': format_diff(diff_passed),
                            'diff_failed': format_diff(diff_failed),
                            'diff_warning': format_diff(diff_warning),
                            'diff_skipped': format_diff(diff_skipped)
            }
        elif model_name in dict1 or model_name in dict2:
            result[model_name] = {
                            'diff_passed': 'N/A',
                            'diff_failed': 'N/A', 
                            'diff_warning': 'N/A', 
