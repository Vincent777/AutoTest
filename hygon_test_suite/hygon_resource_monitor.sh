...
if [ -z $ENGINE_TYPE ]; then
    ...
    echo "Inference Engine Type is wrong! Only SigInfer is supported on Hygon."
    exit 1
fi
...
# 海光环境默认不启用 PD（缺少 pd_place_servers / host_role_lease 时强制关闭）
...
full_model_list_for_smoke=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:8 Qwen3-14B:1)
...
full_model_list_for_accuracy=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:2 Qwen3-14B:1)
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ] && [ ${#servers[@]} -gt 0 ]; then
            ...
            if [ $TEST_TYPE == "Stability" ]; then
                ...
                $curr_dir/siginfer_hygon_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/stability/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_hygon_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${TEST_PARAM} ${version} > $curr_dir/logs/performance/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_hygon_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/smoke/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_hygon_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/accuracy/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
