...
stop_pd_engine_containers() {
    ...
    for ent in "${_pd_stop_ents[@]}"; do
        ...
        suffix="_${pd_id}"
        if [ "$ENGINE_TYPE" = "SigInfer" ]; then
            cname="siginfer_iluvatar_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "vLLM" ]; then
            cname="vllm_iluvatar_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "MindIE" ]; then
            cname="mindie_iluvatar_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "SGLang" ]; then
            cname="sglang_iluvatar_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        else
            continue
        fi
...
cleanup_all_resources() {
    ...
    # 3. 清理远程 Docker 容器（容器名与 job_executor 一致：*_iluvatar_*）
...
if [ -z $version ]; then
    version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/siginfer-aarch64-iluvatar/tags/list | jq -r '.tags[]' \
                        | xargs -I% sh -c "echo -n \"%  \"; \
                            jfrog rt curl --server-id=my-jcr \
                            /api/storage/docker-local/siginfer-aarch64-iluvatar/% \
...
schedule_policies=('PrefillFirst')
ret_code=0
        ...
        for ((j=1; j<=1; j=j+1)); do
            for item in "${model_list[@]}"; do
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ -v pid_map[$done_pid] ]; then
                        ...
                        if [ $err -ne 0 ]; then
                            ...
                            for ip in ${server_list[@]}; do
                                if [ $ENGINE_TYPE == "SigInfer" ]; then
                                    ssh -q -o ConnectionAttempts=3  root@$ip docker stop siginfer_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3  root@$ip docker rm siginfer_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                                elif [ $ENGINE_TYPE == "vLLM" ]; then
                                    ...
                                    ssh -q -o ConnectionAttempts=3  root@$ip docker stop sglang_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3  root@$ip docker rm sglang_iluvatar_${TEST_TYPE}Test_${session_id}_${job_count}
                                fi
                            done
                            fi
                ...
                if [ $TEST_TYPE == "Smoke" ]; then
                    ...
                    bench_container="${engine_type}_iluvatar_PerformanceTest_${session_id}_${job_count}"
                    if [ -n "${PD_TOPOLOGY:-}" ]; then
                ...
                if [ $send_report -eq 1 ]; then
                    ...
                    if [ $TEST_TYPE == "Smoke" ]; then
                        ...
                        mkdir -p ${LOCK_DIR}/artifacts/CI_iluvatar_test/${session_id}/performance
                        cp $curr_dir/report_${log_name_suffix}/${session_id}/* ${LOCK_DIR}/artifacts/CI_iluvatar_test/${session_id}/performance
                    fi
                fi
