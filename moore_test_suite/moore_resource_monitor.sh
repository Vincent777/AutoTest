...
if [ -z $ENGINE_TYPE ]; then
    ...
    echo "Inference Engine Type is wrong! Only SigInfer is supported on Moore."
    exit 1
fi
...
# 摩尔线程环境默认不启用 PD（缺少 pd_place_servers / host_role_lease 时强制关闭）
...
full_model_list_for_smoke=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:2 Qwen3-30B-A3B:2)
full_model_list_for_performance=(DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-0528:16 Qwen3-235B-A22B:8 Qwen3-32B:2 Qwen2.5-72B-Instruct:4 Meta-Llama-3.1-70B-Instruct:4)
...
full_model_list_for_accuracy=(DeepSeek-R1-Distill-Qwen-1.5B:1 DeepSeek-R1-Distill-Qwen-32B:2 DeepSeek-R1-Distill-Llama-8B:1 Meta-Llama-3.1-8B-Instruct:1 Qwen2.5-0.5B-Instruct:1 Qwen2.5-72B-Instruct:4 QwQ-32B:2 Qwen3-32B:2 Qwen3-30B-A3B:2)
full_model_list_for_stability=(DeepSeek-R1-0528:16 Qwen3-235B-A22B:8 Meta-Llama-3.1-70B-Instruct:4 Qwen3-32B:2)
...
search_servers() {
    ...
    for key in "${!npu_server_list[@]}"; do
        ...
        ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@${npu_server_list[$key]} "# 目标空闲 GPU 数量
            if [ -f /home/zkjh/npu_lock_manager_for_ci.sh ]; then
            ...
            BUSY_GPUS=(\$(mthreads-gmi | awk '/^Processes:/{p=1; next} p && \$1 ~ /^[0-9]+\$/ {mem=\$NF; gsub(/MiB/,\"\",mem); if (mem+0 >= 100) print \$1}' | sort -nu))
...
search_pd_servers() {
    ...
    for key in "${!npu_server_list[@]}"; do
        ...
        local probe_out
        probe_out=$(ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 zkjh@${host_ip} "
            if [ -f /home/zkjh/npu_lock_manager_for_ci.sh ]; then
                source /home/zkjh/npu_lock_manager_for_ci.sh
            else
                source /home/zkjh/npu_lock_manager_for_ci.sh
...
for name in "${!npu_server_list[@]}"; do
    ...
    if [ -f "${curr_dir}/pd_ascend_send_kvcache_compat.py" ]; then
        scp "${curr_dir}/pd_ascend_send_kvcache_compat.py" zkjh@${npu_server_list[$name]}:/home/zkjh
    fi
done
    ...
    for item in "${GPU_resource_demand[@]}"; do
        ...
        if [ ${#servers[@]} -ge ${SERVER_QUANTITY} ] && [ ${#servers[@]} -gt 0 ]; then
            ...
            if [ $TEST_TYPE == "Stability" ]; then
                ...
                $curr_dir/siginfer_moore_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/stability/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_moore_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${TEST_PARAM} ${version} > $curr_dir/logs/performance/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_moore_test.sh 1 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/smoke/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
                ...
                PD_TOPOLOGY="$PD_TOPOLOGY" PD_PLACEMENT="$PD_PLACEMENT" \
                $curr_dir/siginfer_moore_test.sh 0 "${servers[*]}" ${model} ${job_count} ${TEST_TYPE} ${ENGINE_TYPE} ${SESSION_ID} ${version} > $curr_dir/logs/accuracy/$SESSION_ID/cron_job_${log_name_suffix}_${job_count}.log 2>&1 &
                last_pid=$!
                pid_map[$last_pid]=$item
