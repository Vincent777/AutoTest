#!/usr/bin/env bash
# Pagoda_HYGON.sh — 海光 SigInfer 一键入口（对齐 ascend Pagoda_910B*.py）
#
# 用法:
#   ./Pagoda_HYGON.sh --test_type Service
#   ./Pagoda_HYGON.sh --test_type Performance
#
# 可选参数:
#   --model MODEL          模型名，默认 DeepSeek-R1-0528；也可用 default
#   --session_id ID        会话 ID，默认 000000
#   --test_param PARAM     Performance 默认 Random（或 SharedGPT）；Service 忽略
#   --version TAG          SigInfer 镜像 tag，默认空（自动取最新 main-*）
#
set -euo pipefail
...
MONITOR="$SCRIPT_DIR/hygon_resource_monitor.sh"
chmod +x "$MONITOR" "$SCRIPT_DIR/siginfer_hygon_test.sh" 2>/dev/null || true

echo "========================================"
echo " Pagoda HYGON / SigInfer"
...
cleanup() {
    echo "Shutting down Pagoda_HYGON..."
...
set +e
# 对齐 ascend: bash hygon_resource_monitor.sh <Stability|Performance> SigInfer <model> <session> [Random] [version]
