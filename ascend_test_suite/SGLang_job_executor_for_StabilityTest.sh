...
if [ $? -eq 0 ]; then
    docker stop sglang_ascend_StabilityTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX}
    docker rm sglang_ascend_StabilityTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX}
fi
...
EXEC_COMMAND="docker run --name=sglang_ascend_StabilityTest_${SESSION_ID}_${JOB_COUNT}${PD_CONTAINER_SUFFIX} \
  --network host \
  --ipc=host \
  --privileged \
