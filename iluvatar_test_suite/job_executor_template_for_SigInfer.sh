...
ret=`docker ps -a | grep siginfer_iluvatar_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_iluvatar_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_iluvatar_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_iluvatar_<<<TEST_TYPE>>>_${SESSION_ID}_${JOB_COUNT} \
     --privileged \
     --ipc=host \
     --network=host \
     --pid=host \
