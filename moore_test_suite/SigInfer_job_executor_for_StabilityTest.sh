...
ret=`docker ps -a | grep siginfer_moore_StabilityTest_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_moore_StabilityTest_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_moore_StabilityTest_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_moore_StabilityTest_${SESSION_ID}_${JOB_COUNT} \
     -u root  \
     --privileged \
     --ipc=host \
     --network host \
