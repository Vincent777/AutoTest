...
ret=`docker ps -a | grep siginfer_ascend_AccuracyTest_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
  docker stop siginfer_ascend_AccuracyTest_${SESSION_ID}_${JOB_COUNT}
  docker rm siginfer_ascend_AccuracyTest_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=siginfer_ascend_AccuracyTest_${SESSION_ID}_${JOB_COUNT} \
     -u root  \
...
if [ $MODEL == "DeepSeek-R1-AWQ" ]; then
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model DeepSeek-R1-Distill-Qwen-14B --tokenizer /home/weight/DeepSeek-R1-Distill-Qwen-14B -tp 2 --block-size 128 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT  --master-addr $MASTER_IP --master-port $MASTER_PORT  --nnodes 1 --node-rank $NODE_RANK --platform-type ascend  $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT"
    EXEC_COMMAND+=" --model DeepSeek-R1-Distill-Qwen-14B --tokenizer /home/weight/DeepSeek-R1-Distill-Qwen-14B -tp 2 --block-size 128 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT  --master-addr $MASTER_IP --master-port $MASTER_PORT  --nnodes 1 --node-rank $NODE_RANK --platform-type ascend  $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT > $LOG_NAME 2>&1 &"
    ...
    echo "SIG_LOG_LEVEL='warn,console_logger=info' ./start.sh --model Qwen3-14B --tokenizer /home/weight/Qwen3/Qwen3-14B -tp 2 --block-size 128 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT  --master-addr $MASTER_IP --master-port $MASTER_PORT  --nnodes 1 --node-rank $NODE_RANK --platform-type ascend  $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT"
    EXEC_COMMAND+=" --model Qwen3-14B --tokenizer /home/weight/Qwen3/Qwen3-14B -tp 2 --block-size 128 --schedule-policy $SCHEDULE_POLICY --max-num-batched-tokens 8192 --port $PORT  --master-addr $MASTER_IP --master-port $MASTER_PORT  --nnodes 1 --node-rank $NODE_RANK --platform-type ascend  $SWAP_SPACE_OPTION --tool-call-parser hermes $USE_PREFIX_CACHE --prometheus-port $PROMETHEUS_PORT > $LOG_NAME 2>&1 &"
fi
