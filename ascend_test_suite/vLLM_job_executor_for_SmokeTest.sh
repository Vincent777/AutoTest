...
ret=`docker ps -a | grep vllm_ascend_SmokeTest_${SESSION_ID}_${JOB_COUNT}`
if [ $? -eq 0 ]; then
    docker stop vllm_ascend_SmokeTest_${SESSION_ID}_${JOB_COUNT}
    docker rm vllm_ascend_SmokeTest_${SESSION_ID}_${JOB_COUNT}
fi
...
EXEC_COMMAND="docker run --name=vllm_ascend_SmokeTest_${SESSION_ID}_${JOB_COUNT} \
  --network host \
  --ipc=host \
  --privileged \
