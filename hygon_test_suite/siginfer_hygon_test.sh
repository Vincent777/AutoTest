...
stop_pd_engine_containers() {
    ...
    for ent in "${_pd_stop_ents[@]}"; do
        ...
        suffix="_${pd_id}"
        if [ "$ENGINE_TYPE" = "SigInfer" ]; then
            cname="siginfer_hygon_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "vLLM" ]; then
            cname="vllm_hygon_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "MindIE" ]; then
            cname="mindie_hygon_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        elif [ "$ENGINE_TYPE" = "SGLang" ]; then
            cname="sglang_hygon_${TEST_TYPE}Test_${session_id}_${jc}${suffix}"
        else
            continue
        fi
        ssh -q -o ConnectionAttempts=3 root@"$pd_host" docker stop "$cname" 2>/dev/null || true
        ssh -q -o ConnectionAttempts=3 root@"$pd_host" docker rm "$cname" 2>/dev/null || true
    ...
    if [ -n "$router_host" ]; then
        ssh -q -o ConnectionAttempts=3 root@"$router_host" \
...
# 标志变量，用于跟踪是否由信号中断 / 是否已清理（避免 SIGINT + EXIT 重复清理）
INTERRUPTED=0
CLEANUP_DONE=0
...
cleanup_all_resources() {
    ...
    # 3. 清理远程 Docker 容器（容器名与 job_executor 一致：*_hygon_*）
    if [ -n "${PD_PLACEMENT:-}" ]; then
        stop_pd_engine_containers "${job_count:-0}"
    else
        for ip in ${server_list[@]}; do
            ssh -q -o ConnectionAttempts=3 root@$ip "
                name=${engine_type}_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
...
if [ -z $version ]; then
    version=$(jfrog rt curl --server-id=my-jcr /api/docker/docker-local/v2/siginfer-aarch64-hygon/tags/list | jq -r '.tags[]' \
                        | xargs -I% sh -c "echo -n \"%  \"; \
                            jfrog rt curl --server-id=my-jcr \
                            /api/storage/docker-local/siginfer-aarch64-hygon/% \
            ...
            for item in "${model_list[@]}"; do
                ...
                launch_executor() {
                    ...
                    if [ $ENGINE_TYPE == "MindIE" ]; then
                        remote_cmd="/root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $slist $rank $job_count $session_id $version"
                    else
                        remote_cmd="/root/${ENGINE_TYPE}_job_executor_for_${TEST_TYPE}Test.sh $model $gpu_quantity $use_prefix_cache_flag $option $swap_space $slist $rank $job_count $session_id $version"
                    fi
                    if [ -n "$PD_TOPOLOGY" ]; then
                    ...
                    if [ $TEST_TYPE == "Smoke" ]; then
                        ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@$ip \
                ...
                while (( remaining > 0 )); do
                    ...
                    if [ -v pid_map[$done_pid] ]; then
                        ...
                        if [ $err -ne 0 ]; then
                            ...
                            for ip in ${server_list[@]}; do
                                if [ $ENGINE_TYPE == "SigInfer" ]; then
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker stop siginfer_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker rm siginfer_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                elif [ $ENGINE_TYPE == "vLLM" ]; then
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker stop vllm_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker rm vllm_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                elif [ $ENGINE_TYPE == "MindIE" ]; then
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker stop mindie_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker rm mindie_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                elif [ $ENGINE_TYPE == "SGLang" ]; then
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker stop sglang_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                    ssh -q -o ConnectionAttempts=3 root@$ip docker rm sglang_hygon_${TEST_TYPE}Test_${session_id}_${job_count}
                                fi
                            done
                            fi
                ...
                if [ $TEST_TYPE == "Smoke" ]; then
                    ...
                    bench_container="${engine_type}_hygon_PerformanceTest_${session_id}_${job_count}"
                    if [ -n "${PD_TOPOLOGY:-}" ]; then
                    ...
                    if [ $TEST_PARAM == "Random" ]; then
                        ...
                        # Random
                        ssh -q -o ConnectionAttempts=3 -o ServerAliveInterval=60 -o ServerAliveCountMax=3 root@${bench_host} "
                            ...
                            docker exec ${bench_container} /bin/bash -c \"
                                ...
                                for concurrency in ${concurrency_list[@]}; do
                                    ...
                                    if [ ${engine_type} == \\\"sglang\\\" ]; then
                                        ...
                                        echo \\\"${benchmark_cmd} --backend openai --port ${server_port} --host ${local_master_ip} --model ${model} --tokenizer ${data_path}/${model}/ --endpoint /v1/completions --dataset-name sharegpt --dataset-path /root/ShareGPT_V3_unfiltered_cleaned_split.json --num-prompts \\\$prompts --request-rate inf --max-concurrency \\\$concurrency\\\"
                                        ${benchmark_cmd} \
                                        --backend openai \
                                        --port ${server_port} \
                                        --host ${local_master_ip} \
                ...
                if [ $send_report -eq 1 ]; then
                    ...
                    if [ $TEST_TYPE == "Smoke" ]; then
                        ...
                        mkdir -p ${LOCK_DIR}/artifacts/CI_hygon_test/${session_id}/performance
                        cp $curr_dir/report_${log_name_suffix}/${session_id}/* ${LOCK_DIR}/artifacts/CI_hygon_test/${session_id}/performance
                    fi
                fi
